...
from model.visual_encoder import visual_encoder
...
class MultiModalSwinTransformer(nn.Module):
    def __init__(self,
        ...
        self.garan = GaranAttention(15360, 768, 2)
        # self.garans = nn.ModuleList()
        # for i_garan in out_indices:
        #     garan = GaranAttention(15360, 96*(2**i_garan), 2)
        #     self.garans.append(garan)
    ...
    def forward(self, x, l, l_mask):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                lx = l.contiguous().view(l.shape[0], -1)
                if i == 3:
                    # garan = self.garans[i]
                    # xx, m_attn, attn = garan(lx, out)
                    xx, m_attn, attn = self.garan(lx, out)
                    # xx, m_attn, attn = garan(lx, out)
                # out = attn
                outs.append(out)
